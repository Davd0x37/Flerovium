...
import { Service, DataPath, Data } from '@/types/service';
...
const selectData = (
  obj: Selector,
  selectors: Data[],
): Pick<Data, 'label' | 'detail' | 'isEnabled' | 'isImportant'>[] =>
  ...
  async authenticate(service: Service): Promise<string> {
    try {
      ...
      const { codeVerifier, codeChallenge, authorizationUri } =
        await oauthStrategy.setupAuthentication();
