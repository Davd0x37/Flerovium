...
import { encrypt, functions, decrypt } from '@flerovium/crypto';
...
export const downloadVault = async (vault: RootState): Promise<URLDownload> => {
  try {
    ...
    const restABEncrypted = await encrypt(restAB, rest.encryption.passwordHash);
...
export const openVault = async (
  ...
  try {
    ...
    const passwordHash = await functions.Argon2.hash(
      rawPassword,
      encryption.salt,
    );
    ...
    const decryptedAB = await decrypt(dataFromHex, passwordHash?.encoded!);
